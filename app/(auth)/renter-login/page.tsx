...
import { Mail, Lock, EyeOff, Eye, ArrowLeft, Loader2, KeyRound, CheckCircle2 } from "lucide-react";
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4 py-12 sm:px-6 lg:px-8">
      <div className="w-full max-w-md space-y-8 border border-border bg-card p-8 shadow-2xl sm:rounded-3xl relative overflow-hidden">
        <div className="flex flex-col space-y-2 text-center">
          ...
          >
            {showPasswordLogin ? "Welcome Back" : 
             step === "code" ? "Check your Email" :
             step === "password" ? "Secure Account" : "Renter Login"}
          </h1>
          <p className="text-muted-foreground">
            {showPasswordLogin
              ? "Sign in with your email and password." :
             step === "code" ? `We sent a code to ${tempEmail}` :
        ...
        <AnimatePresence mode="wait">
          {showPasswordLogin ? (
            ...
            >
              ...
              <form onSubmit={handleEmailStep} className="grid gap-2">
                <div className="relative">
                  <Mail className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                  <input
                    type="email"
                ...
                >
                  {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Sign In with Email"}
                </button>
              </form>
              ...
              <div className="grid gap-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="h-12 w-full rounded-full border border-border bg-background pl-11 pr-4 text-center text-lg font-bold tracking-[0.5em] transition-all focus:border-accent focus:ring-2 focus:ring-accent/20 outline-none"
                  />
                </div>
              </div>
              <button
                disabled={isLoading}
                type="submit"
                className="h-12 w-full flex items-center justify-center rounded-full bg-accent font-bold text-accent-foreground transition-transform hover:scale-[1.02] active:scale-95 disabled:opacity-50"
              >
            ...
            <motion.form
              ...
              className="grid gap-4 py-4"
              onSubmit={handleSetupPasswordStep}
            >
              <div className="bg-accent/10 border border-accent/20 p-4 rounded-2xl flex items-start gap-3 mb-2">
                <CheckCircle2 className="h-5 w-5 text-accent shrink-0 mt-0.5" />
                <p className="text-xs text-accent-foreground leading-relaxed">
                  Verification successful! Please set a password to secure your account for future logins.
                </p>
              </div>
              <div className="grid gap-2">
                <div className="relative">
